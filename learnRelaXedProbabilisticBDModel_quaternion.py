...
from binDeltaGenerators import XPBDGeneratorQ
...
from binDeltaLosses import RelaXedProbabilisticLossQ, RelaXedProbabilisticMultiresLossQ
...
if not args.multires:
	criterion = RelaXedProbabilisticLossQ(1.0, kmeans_file, geodesic_loss(reduce=False).cuda())
else:
	criterion = RelaXedProbabilisticMultiresLossQ(1.0, kmeans_file, geodesic_loss(reduce=False).cuda())
...
def testing():
	...
	for i, sample in enumerate(test_loader):
		...
		if not args.multires:
			...
			ypred_bin = np.argmax(output[0].data.cpu().numpy(), axis=1)
			ypred_res = output[1].data.cpu().numpy()
			y = np.stack([kmeans_dict[ypred_bin[i]] + ypred_res[i, ypred_bin[i], :] for i in range(ypred_bin.shape[0])])
