...
class JointCatPoseModel(nn.Module):
	...
	def forward(self, x):
		...
		for i in range(self.num_classes):
			ybin = self.bin_models[i](x1)
			ind = torch.argmax(ybin, dim=1)
			if not args.multires:
				yres = self.res_models[i](x1)
			else:
				...
				for j in range(self.num_clusters):
					yres.append(self.res_models[i * self.num_clusters + j](x1))
...
spio.savemat(results_file, {'pose_results': pose_results, 'top1_results': top1_results, 'wgt_results': wgt_results})
