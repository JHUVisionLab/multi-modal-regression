...
class JointCatPoseModel(nn.Module):
	def __init__(self, oracle_model, resnet_model):
		super().__init__()
		# old stuff
		...
		# new stuff
		self.category_model = resnet_model.layer4
		self.avgpool = resnet_model.avgpool
		self.fc = nn.Linear(2048, num_classes).cuda()

	def forward(self, x):
		x = self.feature_model.features(x)
		# category branch
		x0 = self.category_model(x)
		x0 = self.avgpool(x0)
		x0 = x0.view(x0.size(0), -1)
		y0 = self.fc(x0)
		label = torch.unsqueeze(F.softmax(y0, dim=1), dim=2)
		# pose branch
		x1 = self.feature_model.avgpool(x)
		x1 = x1.view(x1.size(0), -1)
		if not args.multires:
			y1 = torch.stack([self.bin_models[i](x1) for i in range(self.num_classes)]).permute(1, 2, 0)
			y2 = torch.stack([self.res_models[i](x1) for i in range(self.num_classes)]).permute(1, 2, 0)
			y1 = torch.squeeze(torch.bmm(y1, label), 2)
			y2 = torch.squeeze(torch.bmm(y2, label), 2)
		else:
			y1 = torch.stack([self.bin_models[i](x1) for i in range(self.num_classes)]).permute(1, 2, 0)
			y2 = torch.stack([self.res_models[i](x1) for i in range(self.num_classes * self.num_clusters)])
...
model = JointCatPoseModel(orig_model, resnet_model)
# print(model)
